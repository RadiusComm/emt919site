...
The `/educators` landing page uses `educators.20260918-refined.css` and `educators.20260918-refined.js`, with responsive illustrative images for the family, classroom, and school-office scenes. Its school-labelled inquiry form reuses the registered `emt919-demo` field names and submission handler, with `/educators-thanks` as its confirmation page. Four fields are initially visible; phone, organization size, and message remain in an optional disclosure. `#school-demo` targets the form directly. A mobile CTA appears beyond the hero and hides when the form, footer, or a text-entry control is in use. Existing agency inquiry forms retain `/thanks`. Navigation templates and the shell synchronizer preserve the education page's school-demo links and footer summary.

The education comparison at `#why-emt919` uses `educators-security.20260918-button-fix.css`. It describes secure teacher transcript delivery and authorized administrative oversight using the site owner's education workflow, and encryption, verification, logging, and AI data-use controls from the Trust Center. Avoid blanket claims about all free apps; features vary by provider and plan. `#districts` remains an alias for this section.
